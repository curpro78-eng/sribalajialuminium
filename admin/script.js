...
var EM={"New Launch":"🆕","Aluminium":"🍳","Stainless Steel":"🔘","Hotelware":"🏨","Pooja Articles":"🪔","Appliances & Brands":"⚡","Water Bottles":"💧","Household":"🏠","Cutlery":"🍴","Kitchenware":"🥘"};

/* APPLIANCES LOGIC */
const APP_BRANDS = {
  "Cookers": ["🟡 Prestige", "🦋 Butterfly", "🔵 Ultra", "⭐ Hawkins", "💎 Diamond"],
  "Mixie": ["🔴 Preethi", "🔵 Ultra", "🟢 Vidiem"],
  "Wet Grinder": ["🔵 Ultra", "🟠 Vijayalakshmi", "🟣 Amirthaa"]
};
...
function formatCategoryLabel(p) {
  var cat = p.category || '';
  if (cat === 'Appliances & Brands' && p.applianceType) {
...
function renderRecent(){
  ...
  container.innerHTML=items.map(function(p){
    ...
    return'<div class="recent-item">'+img+'<div class="ri-name">'+eh(p.productName)+'</div><span class="cat-pill">'+eh(formatCategoryLabel(p))+'</span><span class="ri-price">'+(p.price>0?'₹'+p.price:'On Request')+'</span><span class="status-badge '+(p.available!==false?'avail':'unavail')+'">'+(p.available!==false?'✅ Active':'❌ Hidden')+'</span></div>';
  }).join('');
}
...
function submitProduct(e){
  ...
  var data={productName:document.getElementById('pName').value.trim(),category:cat,applianceType:aType,brand:brand,price:parseFloat(document.getElementById('pPrice').value)||0,description:document.getElementById('pDesc').value.trim(),aliases:aliases,imageUrl:addImgData||'',available:document.getElementById('pAvail').checked,sortOrder:parseInt(document.getElementById('pSort').value)||allProds.length,createdAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
...
function saveEdit(e){
  ...
  var data={productName:document.getElementById('eName').value.trim(),category:cat,applianceType:aType,brand:brand,price:parseFloat(document.getElementById('ePrice').value)||0,description:document.getElementById('eDesc').value.trim(),aliases:aliases,available:document.getElementById('eAvail').checked,sortOrder:parseInt(document.getElementById('eSort').value)||0,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
