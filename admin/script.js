...
var EM={"New Launch":"🆕","Aluminium":"🍳","Stainless Steel":"🔘","Hotelware":"🏨","Pooja Articles":"🪔","Appliances":"⚡","Copper Utensils":"🥘","Brass Utensils":"🥘","Cutlery":"🍴","Kitchenware":"🥘"};

/* APPLIANCES LOGIC */
const APP_BRANDS = {
  "Cookers": ["🟡 Prestige", "🦋 Butterfly", "🔵 Ultra", "⭐ Hawkins", "💎 Diamond", "📦 Others"],
  "Mixie": ["🔴 Preethi", "🔵 Ultra", "🟢 Vidiem", "🦋 Butterfly", "🟠 Vijayalakshmi", "📦 Other"],
  "Wet Grinder": ["🔵 Ultra", "🟠 Vijayalakshmi", "🟣 Amirthaa", "🦋 Butterfly", "📦 Others"],
  "Gas Stove": ["🟡 Prestige", "🦋 Butterfly", "🔴 Preethi", "🟢 Vidiem", "🟠 Vijayalakshmi", "🟣 Amirthaa", "📦 Other"],
  "Induction Stove": ["🟡 Prestige", "🦋 Butterfly", "🔴 Preethi", "📦 Other"],
  "Flask": ["🦋 Butterfly", "💎 Diamond", "🎒 Milton", "📦 Other"],
  "Others": ["📦 Others"]
};

function handleCategory(prefix) {
  var cat = document.getElementById(prefix + 'Cat').value;
  var row = document.getElementById(prefix + 'AppRow');
  if (cat === 'Appliances & Brands') {
    row.style.display = '';
  } else {
    row.style.display = 'none';
    document.getElementById(prefix + 'AppType').value = '';
    document.getElementById(prefix + 'Brand').innerHTML = '<option value="">Select Brand</option>';
  }
}

function handleApplianceType(prefix) {
  var type = document.getElementById(prefix + 'AppType').value;
  var brandSel = document.getElementById(prefix + 'Brand');
  brandSel.innerHTML = '<option value="">Select Brand</option>';
  if (APP_BRANDS[type]) {
    APP_BRANDS[type].forEach(function(b) {
...
function formatCategoryLabel(p) {
  ...
  if ((cat === 'Appliances & Brands' || cat === 'Appliances') && p.applianceType) {
    return '⚡ ' + p.applianceType + (p.brand ? (' - ' + (p.brand.split(' ')[1] || p.brand)) : '');
  }
  var e = EM[p.category] || '🪣';
  return e + ' ' + cat;
}
...
function renderRecent(){
  ...
  container.innerHTML=items.map(function(p){
    ...
    var pStr = p.price && String(p.price).trim() !== '' ? String(p.price).trim() : '';
    var prH = pStr !== '' ? eh(pStr) : 'On Request';
    return'<div class="recent-item">'+img+'<div class="ri-name">'+eh(p.productName)+'</div><span class="cat-pill">'+eh(formatCategoryLabel(p))+'</span><span class="ri-price">'+prH+'</span><span class="status-badge '+(p.available!==false?'avail':'unavail')+'">'+(p.available!==false?'✅ Active':'❌ Hidden')+'</span></div>';
  }).join('');
}
...
function filterTable(){
  ...
  filteredProds=allProds.filter(function(p){
    var combinedStr = (p.productName||'') + ' ' + (p.category||'') + ' ' + (p.applianceType||'') + ' ' + (p.brand||'');
    var mq=!q||combinedStr.toLowerCase().includes(q);
    var mc=false;
    if(!cat){ mc=true; }
    else{
      var pCat=(p.category||'').trim().toLowerCase();
      var fCat=cat.trim().toLowerCase();
      mc=(pCat===fCat) || (fCat==='appliances & brands' && pCat==='appliances') || (fCat==='appliances' && pCat==='appliances & brands');
    }
    return mq&&mc;
  });renderTable();
}
...
function renderTable(){
  ...
  tb.innerHTML=filteredProds.map(function(p,idx){
    ...
    return'<tr draggable="true" data-id="'+p.id+'" data-idx="'+idx+'"'
      ...
      +'<td><span class="cat-pill">'+eh(formatCategoryLabel(p))+'</span></td>'
      +'<td style="font-weight:700;color:var(--gold3)">'+prH+'</td>'
...
function submitProduct(e){
  ...
  var aType = cat === 'Appliances & Brands' ? document.getElementById('pAppType').value : '';
  var brand = cat === 'Appliances & Brands' ? document.getElementById('pBrand').value : '';
  if (cat === 'Appliances & Brands' && (!aType || !brand)) {
    showToast('❌ Please select Appliance Type and Brand', 'err');
    btn.textContent='💾 Save Product';btn.disabled=false;
    return;
  }
  var data={productName:document.getElementById('pName').value.trim(),category:cat,applianceType:aType,brand:brand,price:document.getElementById('pPrice').value.trim(),description:document.getElementById('pDesc').value.trim(),aliases:aliases,imageUrl:addImgData||'',available:document.getElementById('pAvail').checked,sortOrder:parseInt(document.getElementById('pSort').value)||allProds.length,createdAt:firebase.firestore.FieldValue.serverTimestamp(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
...
function resetAddForm(){
  ...
  var row = document.getElementById('pAppRow'); if(row) row.style.display='none';
  var pBrand = document.getElementById('pBrand'); if(pBrand) pBrand.innerHTML='<option value="">Select Brand</option>';
  document.getElementById('imgPrev').style.display='none';addImgData=null;var p=document.querySelector('#uploadArea p');if(p)p.innerHTML='<strong>Click to upload image</strong><br><small>JPG, PNG, WebP – auto-compressed</small>';
}
...
function openEdit(id){
  ...
  var cat = p.category||'Kitchenware';
  document.getElementById('eCat').value=cat;
  if(cat === 'Appliances & Brands'){
    document.getElementById('eAppRow').style.display='';
    document.getElementById('eAppType').value = p.applianceType||'';
    handleApplianceType('e');
    setTimeout(function() { document.getElementById('eBrand').value = p.brand||''; }, 10);
  } else {
    document.getElementById('eAppRow').style.display='none';
    document.getElementById('eAppType').value = '';
    document.getElementById('eBrand').innerHTML='<option value="">Select Brand</option>';
  }
...
function saveEdit(e){
  ...
  var aType = cat === 'Appliances & Brands' ? document.getElementById('eAppType').value : '';
  var brand = cat === 'Appliances & Brands' ? document.getElementById('eBrand').value : '';
  if (cat === 'Appliances & Brands' && (!aType || !brand)) {
    showToast('❌ Please select Appliance Type and Brand', 'err');
    return;
  }
  var data={productName:document.getElementById('eName').value.trim(),category:cat,applianceType:aType,brand:brand,price:document.getElementById('ePrice').value.trim(),description:document.getElementById('eDesc').value.trim(),aliases:aliases,available:document.getElementById('eAvail').checked,sortOrder:parseInt(document.getElementById('eSort').value)||0,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
...
/* EXPORT / IMPORT JSON */
function exportJson() {
  var data = JSON.stringify(allProds, null, 2);
...
function importJson(input) {
  ...
  reader.onload = function(e) {
    try {
      var importedProds = JSON.parse(e.target.result);
      if(!Array.isArray(importedProds)) throw new Error("Invalid format, expected array.");
      
      var newNames = importedProds.map(function(p){ return p.productName });
      var existingNames = allProds.map(function(p){ return p.productName });
      
      var batch = db.batch();
      var count = 0;
      
      importedProds.forEach(function(p) {
        if(existingNames.includes(p.productName)) return; // skip duplicates
        var ref = col.doc();
        var data = {
          ...
          brand: p.brand || '',
          price: p.price !== undefined ? String(p.price).trim() : '',
      ...
      if(count > 0) {
        batch.commit().then(function() {
          showToast('✅ Imported ' + count + ' products successfully!', 'ok');
        }).catch(function(err) {
          showToast('❌ Import Error: ' + err.message, 'err');
        });
      } else {
        showToast('ℹ️ No new products to import (duplicates skipped).', 'ok');
      }
    } catch(err) {
      showToast('❌ JSON Parse Error: ' + err.message, 'err');
    }
    input.value = "";
  };
