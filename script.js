...
var customFilterActive=false,activeApplianceType=null,activeBrand=null;

var EM={Aluminium:'🍳','Stainless Steel':'🔘',Brass:'🔶','Brass Utensils':'🥘',Copper:'🟤','Copper Utensils':'🥘','Water Bottles':'💧',Household:'🏠',Kitchenware:'🥘',Hotelware:'🏨','Pooja Articles':'🪔','New Launch':'🆕',Cutlery:'🍴',Appliances:'⚡'};
var BC={Aluminium:'b-al','Stainless Steel':'b-ss',Brass:'b-br','Brass Utensils':'b-br',Copper:'b-cu','Copper Utensils':'b-cu','Water Bottles':'b-cu',Household:'b-br',Kitchenware:'b-kw',Hotelware:'b-hw','Pooja Articles':'b-pa','New Launch':'b-nl',Cutlery:'b-ss',Appliances:'b-ap'};

function formatCategoryLabel(p) {
  var cat = p.category || '';
  if (cat === 'Appliances' && p.applianceType) {
...
function navFilterProducts(applianceType, brand){filterProducts(applianceType, brand);if(document.getElementById('mobNav').classList.contains('open'))toggleMob()}
...
function filterProducts(applianceType = null, brand = null) {
  customFilterActive = true;
  activeApplianceType = applianceType;
  activeBrand = brand;
  searchQuery = '';
  
  // Update UI tabs to Appliances
  document.querySelectorAll('.fb').forEach(function(t){t.classList.remove('act')});
  var appBtn = document.querySelector('.fb[data-cat="Appliances & Brands"]') || document.querySelector('.fb[data-cat="Appliances"]');
  if(appBtn) appBtn.classList.add('act');
  
  // Clear search inputs
  var di = document.getElementById('deskSearchInp'); if(di) di.value = '';
  var dClr = document.getElementById('deskSearchClear'); if(dClr) dClr.style.display = 'none';
  var mi = document.getElementById('mobSearchInpSticky'); if(mi) mi.value = '';
  var mClr = document.getElementById('mobSearchClear'); if(mClr) mClr.style.display = 'none';
  
  closeDeskDD();
  var mdd = document.getElementById('mobSearchDD'); if(mdd) { mdd.classList.remove('open'); mdd.innerHTML=''; }
...
function getFilteredProds(){
  ...
  if (customFilterActive) {
    list = allProducts;
    if (activeApplianceType) {
      list = list.filter(function(p) { return p.applianceType === activeApplianceType; });
    }
    if (activeBrand) {
      list = list.filter(function(p) { return p.brand && p.brand.toLowerCase().includes(activeBrand.toLowerCase()); });
    }
  } else {
    list = allProducts.filter(function(p){
      ...
      if(activeCat==='All') {
        catMatch=true;
      } else if(activeCat==='New Launch') {
        catMatch=p.category==='New Launch';
      } else {
        catMatch=p.category && p.category.toLowerCase()===activeCat.toLowerCase();
      }
  ...
  else if(sort==='pa')list=[].concat(list).sort(function(a,b){return(parseFloat(String(a.price).replace(/[^\d.]/g,''))||0)-(parseFloat(String(b.price).replace(/[^\d.]/g,''))||0)});
  else if(sort==='pd')list=[].concat(list).sort(function(a,b){return(parseFloat(String(b.price).replace(/[^\d.]/g,''))||0)-(parseFloat(String(a.price).replace(/[^\d.]/g,''))||0)});
  return list;
}
...
function updateSectionHeader(){
  ...
  if(customFilterActive) {
    ey.textContent = activeApplianceType ? activeApplianceType + (activeBrand ? ' - ' + activeBrand : '') : 'Appliances';
    ti.innerHTML = 'Premium <span class="gold">Appliances</span>';
  } else if(searchQuery){
...
/* ── Render Products ── */
var curRenderCount = 0;
var currentRenderList = [];

function renderProds(append = false){
  if(!append){
    currentRenderList=getFilteredProds();
    curRenderCount=0;
    updateSectionHeader();
    ...
    g.innerHTML=''; // clear it
  }

  var listToRender = currentRenderList.slice(curRenderCount, curRenderCount + 12);
  ...
  var html = listToRender.map(function(p,i){
  ...
  curRenderCount += listToRender.length;
  initReveal();
...
function setupInfiniteScroll() {
  ...
  var sentinel = document.getElementById('scrollSentinel');
  if(!sentinel){
    sec.insertAdjacentHTML('beforeend', '<div id="scrollSentinel" style="height:40px;width:100%;margin-top:20px;"></div>');
    sentinel = document.getElementById('scrollSentinel');
  }
  
  if (curRenderCount >= currentRenderList.length) {
...
function openProdModal(id){
  ...
  var pStr = p.price && String(p.price).trim() !== '' ? String(p.price).trim() : '';
  var prH=pStr !== ''?'<div class="modal-price">'+eh(pStr)+'</div>':'<div class="modal-price req">📞 Price on Request — Call for Quote</div>';
...
function renderCart(){
  ...
  c.innerHTML=cart.map(function(item){
    ...
    var pStr = item.price && String(item.price).trim() !== '' ? eh(item.price) : '';
    var pr = pStr !== '' ? pStr + ' (x'+item.qty+')' : 'Qty: '+item.qty;
  ...
  var total=cart.reduce(function(s,i){
    var n = parseFloat(String(i.price).replace(/[^\d.]/g,''))||0;
    return s+(n*i.qty);
  },0);
...
function checkoutWA(){
  ...
  cart.forEach(function(item,i){
    var pStr = item.price && String(item.price).trim() !== '' ? ' (' + String(item.price) + ')' : '';
    msg+=(i+1)+'. '+item.name+' – Qty: '+item.qty+pStr+'\n';
  });
  var total=cart.reduce(function(s,i){
    var n = parseFloat(String(i.price).replace(/[^\d.]/g,''))||0;
    return s+(n*i.qty);
